...
namespace Slic3r
{
        ...
        for (auto expansion_iterator = bridge_expansions.begin(); expansion_iterator != bridge_expansions.end();)
        {
            ...
            for (; expansion_iterator != boundary_region_end; ++expansion_iterator)
            {
                ...
                for (; candidate_iterator != boundary_region_end; ++candidate_iterator)
                {
                    ...
                    if (
                        expansion_iterator->src_id != candidate_iterator->src_id && current_bounding_box.overlap(candidate_bounding_box)
    ...
    void LayerRegion::process_external_surfaces(const Layer *lower_layer, const Polygons *lower_layer_covered)
    {
        ...
        SurfaceCollection bridges;
        {
            ...
            bridges.surfaces = custom_angle > 0 ? expand_merge_surfaces(fill_surfaces.surfaces, stBottomBridge, expansion_zones, closing_radius, Geometry::deg2rad(custom_angle)) : expand_bridges_detect_orientations(fill_surfaces.surfaces, expansion_zones, closing_radius);
