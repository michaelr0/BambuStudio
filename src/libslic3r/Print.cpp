...
namespace Slic3r
{
    ...
    bool Print::invalidate_state_by_config_options(const ConfigOptionResolver & /* new_config */, const std::vector<t_config_option_key> &opt_keys)
    {
        ...
        for (const t_config_option_key &opt_key : opt_keys)
        {
            ...
            else if (opt_key == "filament_soluble" || opt_key == "filament_is_support" || opt_key == "filament_printable" || opt_key == "impact_strength_z" || opt_key == "filament_scarf_seam_type" || opt_key == "filament_scarf_height" || opt_key == "filament_scarf_gap" || opt_key == "filament_scarf_length" || opt_key == "filament_change_length" || opt_key == "independent_support_layer_height" || opt_key == "top_z_overrides_xy_distance" || opt_key == "filament_change_length_nc")
            {
                ...
                opt_key == "initial_layer_line_width" || opt_key == "min_layer_height" || opt_key == "max_layer_height"
                //|| opt_key == "resolution"
                // BBS: when enable arc fitting, we must re-generate perimeter
                || opt_key == "enable_arc_fitting" || opt_key == "wall_sequence" || opt_key == "z_direction_outwall_speed_continuous" || opt_key == "override_filament_scarf_seam_setting" || opt_key == "seam_slope_type" || opt_key == "seam_slope_start_height" || opt_key == "seam_slope_gap" || opt_key == "seam_slope_min_length" || opt_key == "embedding_wall_into_infill")
            {
    ...
    std::vector<size_t> Print::layers_sorted_for_object(float start, float end, std::vector<LayerPtrs> &layers_of_objects, std::vector<BoundingBox> &boundingBox_for_objects, std::vector<Points> &objects_instances_shift)
    {
        ...
        std::sort(idx_of_object_sorted.begin(), idx_of_object_sorted.end(),
                  [boundingBox_for_objects](auto left, auto right)
    ...
    for (auto &inst : print_instance_with_bounding_box)
        ...
        // sort the print instance
        std::sort(print_instance_with_bounding_box.begin(), print_instance_with_bounding_box.end(),
                  [](print_instance_info &l, print_instance_info &r)
                  { return l.object_index < r.object_index; });
    ...
    StringObjectException Print::check_multi_filament_valid(const Print &print)
    {
        ...
        { // use ByObject valid under ByObject print sequence
            ...
            { // at least one object has HighLowMixed
            ...
            { // at least one object has other Mixed
                ret.is_warning = true;
                ret.hypetext = hypertext;
...
BoundingBox Print::total_bounding_box() const
{
    ...
    void Print::process(std::unordered_map<std::string, long long> *slice_time, bool use_cache)
    {
        ...
        if (this->set_started(psWipeTower))
        {
            ...
            else if (this->config().print_sequence != PrintSequence::ByObject || (this->config().print_sequence == PrintSequence::ByObject && m_objects.size() == 1))
            {
        ...
        for (PrintObject *obj : m_objects)
        {
            if (((!use_cache) && (need_slicing_objects.count(obj) != 0)) || (use_cache && (m_reslicing_objects.count(obj) != 0)))
            {
    ...
    int Print::load_cached_data(const std::string &directory)
    {
        ...
        for (int obj_index = 0; obj_index < object_jsons.size(); obj_index++)
        {
            ...
            try
            {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(":will load %1%, identify_id %2%, layer_count %3%, support_layer_count %4%, firstlayer_group_count %5%") % name % identify_id % layer_count % support_layer_count % firstlayer_group_count;
                ...
                for (int index = 0; index < layer_count; index++)
                {
                    ...
                    for (int region_index = 0; region_index < layer_regions_count; region_index++)
                    {
                        ...
                        if (!print_region)
                        {
                            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(":can not find print region of object %1%, layer %2%, print_z %3%, layer_region %4%") % name % index % new_layer->print_z % region_index;
                            // delete new_layer;
                ...
                for (int index = 0; index < firstlayer_group_count; index++)
                {
                    ...
                    for (ObjectID &obj_id : firstlayer_group.volume_ids)
                    {
                        ...
                        {
                            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(": can not find volume_id %1% from object file %2% in firstlayer groups, volume_count %3%!") % obj_id.id % object_filenames[obj_index].first % volume_count;
