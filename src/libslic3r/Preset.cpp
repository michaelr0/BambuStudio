...
namespace Slic3r
{
    ...
    ConfigFileType guess_config_file_type(const ptree &tree)
    {
        ...
        return (app_config > bundle && app_config > config) ? CONFIG_FILE_TYPE_APP_CONFIG : (bundle > config) ? CONFIG_FILE_TYPE_CONFIG_BUNDLE
    ...
    std::map<std::string, std::string> VendorProfile::PrinterModel::get_bed_texture_maps() const
    {
        ...
        if (use_double_extruder_default_texture.size() > 0)
        {
            maps["use_double_extruder_default_texture"] = use_double_extruder_default_texture;
        }
        if (bottom_texture_end_name.size() > 0)
        {
            maps["bottom_texture_end_name"] = bottom_texture_end_name;
        }
        if (bottom_texture_rect.size() > 0)
        {
            maps["bottom_texture_rect"] = bottom_texture_rect;
        }
        if (bottom_texture_rect_longer.size() > 0)
        {
            maps["bottom_texture_rect_longer"] = bottom_texture_rect_longer;
        }
        if (middle_texture_rect.size() > 0)
        {
            maps["middle_texture_rect"] = middle_texture_rect;
        }
        return maps;
    }
    ...
    static std::vector<std::string> s_Preset_print_options{
        ...
        "wall_sequence", "is_outer_second", "is_infill_first", "sparse_infill_density", "fill_multiline", "sparse_infill_pattern", "sparse_infill_anchor", "sparse_infill_anchor_max", "top_surface_pattern",
    ...
    PresetCollection::PresetCollection(Preset::Type type, const std::vector<std::string> &keys, const Slic3r::StaticPrintConfig &defaults, const std::string &default_name) : m_type(type),
    ...
    void PresetCollection::load_project_embedded_presets(std::vector<Preset *> &project_presets, const std::string &type, PresetsConfigSubstitutions &substitutions, ForwardCompatibilitySubstitutionRule rule)
    {
        ...
        for (it = project_presets.begin(); it != project_presets.end(); it++)
        {
            ...
            try
            {
                ...
                std::string incorrect_keys = Preset::remove_invalid_keys(preset->config, default_preset.config);
                if (!incorrect_keys.empty())
                    BOOST_LOG_TRIVIAL(error) << "Error in a preset file: The preset \"" << preset->name << "\" contains the following incorrect keys: " << incorrect_keys << ", which were removed";
                preset->loaded = true;
                presets_loaded.emplace_back(*preset);
    ...
    bool PresetCollection::load_user_preset(std::string name, std::map<std::string, std::string> preset_values, PresetsConfigSubstitutions &substitutions, ForwardCompatibilitySubstitutionRule rule)
    {
        ...
        try
        {
            ...
            if (!incorrect_keys.empty())
                BOOST_LOG_TRIVIAL(error) << "Error in a preset file: The preset \"" << name << "\" contains the following incorrect keys: " << incorrect_keys << ", which were removed";
            if (need_update)
            {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(", update the user preset %1% from cloud, type %2%, setting_id %3%, base_id %4%, sync_info %5% inherits %6%, filament_id %7%") % iter->name % Preset::get_type_string(m_type) % iter->setting_id % iter->base_id % iter->sync_info % iter->inherits() % iter->filament_id;
            }
            else
            {
                // create a new one
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(", insert a new user preset %1%, type %2%, setting_id %3%, base_id %4%, sync_info %5% inherits %6%, filament_id %7%") % preset.name % Preset::get_type_string(m_type) % preset.setting_id % preset.base_id % preset.sync_info % preset.inherits() % preset.filament_id;
    ...
    const std::string &PresetCollection::get_preset_name_by_alias(const std::string &alias) const
    {
        for (
            ...
            auto it = Slic3r::lower_bound_by_predicate(m_map_alias_to_profile_name.begin(), m_map_alias_to_profile_name.end(), [&alias](auto &l)
                                                       { return l.first < alias; });
    ...
    Preset *PresetCollection::find_preset(const std::string &name, bool first_visible_if_not_found, bool real)
    {
        ...
        return (it != m_presets.end() && it->name == key.name) ? &this->preset(it - m_presets.begin(), real) : first_visible_if_not_found ? &this->first_visible()
                                                                                                                                          : nullptr;
    }
        ...
        for (const t_config_option_key &opt_key : keys)
        {
            ...
            if (this_opt != nullptr && other_opt != nullptr && *this_opt != *other_opt)
            {
                ...
                else
                {
                    ...
                    {
                    case coInts:
                        add_correct_opts_to_diff<ConfigOptionInts>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coBools:
                        add_correct_opts_to_diff<ConfigOptionBools>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coFloats:
                        add_correct_opts_to_diff<ConfigOptionFloats>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coStrings:
                        add_correct_opts_to_diff<ConfigOptionStrings>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coPercents:
                        add_correct_opts_to_diff<ConfigOptionPercents>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coFloatsOrPercents:
                        add_correct_opts_to_diff<ConfigOptionFloatsOrPercents>(opt_key, diff, config_other, config_this, strict);
                        break;
                    case coPoints:
                        add_correct_opts_to_diff<ConfigOptionPoints>(opt_key, diff, config_other, config_this, strict);
                        break;
                    // BBS
                    case coEnums:
                        add_correct_opts_to_diff<ConfigOptionInts>(opt_key, diff, config_other, config_this, strict);
...
// BBS
    ...
    {
        if (case_sensitive_search)
            return Slic3r::lower_bound_by_predicate(m_printers.begin(), m_printers.end(), [&name](const auto &l)
                                                    { return l.name < name; });
    ...
    namespace PresetUtils
    {
        const VendorProfile::PrinterModel *system_printer_model(const Preset &preset)
        {
            ...
            if (preset.vendor != nullptr)
            {
                ...
                {
                    auto it = std::find_if(preset.vendor->models.begin(), preset.vendor->models.end(), [printer_model](const VendorProfile::PrinterModel &pm)
