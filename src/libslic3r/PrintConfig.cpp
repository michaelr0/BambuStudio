...
#define CONFIG_OPTION_ENUM_DEFINE_STATIC_MAPS(NAME)                                                     \
    ...
    template <>                                                                                         \
    const t_config_enum_values &ConfigOptionEnum<NAME>::get_enum_values() { return s_keys_map_##NAME; } \
    template <>                                                                                         \
    const t_config_enum_names &ConfigOptionEnum<NAME>::get_enum_names() { return s_keys_names_##NAME; }
    ...
    void PrintConfigDef::init_fff_params()
    {
      ...
      // BBS: The following two types are disabled
        ...
        def->enum_values.push_back("inner-outer-inner-inner wall");
        ...
        def->category = L("Quality");
        def->tooltip = L("The wipe speed is determined by speed of current extrusion role. "
...
//    def->min = 0;
    ...
    //def->enum_labels.push_back(L("0.2 (detachable)"));
        ...
        for (auto &opt_key : filament_extruder_override_keys)
        {
            ...
            {
            case coFloats:
                def->set_default_value(new ConfigOptionFloatsNullable(static_cast<const ConfigOptionFloatsNullable *>(it_opt->second.default_value.get())->values));
                break;
            case coPercents:
                def->set_default_value(new ConfigOptionPercentsNullable(static_cast<const ConfigOptionPercentsNullable *>(it_opt->second.default_value.get())->values));
                break;
            case coBools:
                ...
                break;
            case coEnums:
                def->set_default_value(new ConfigOptionEnumsGenericNullable(static_cast<const ConfigOptionEnumsGenericNullable *>(it_opt->second.default_value.get())->values));
                break;
            default:
                assert(false);
            }
    ...
    void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
    {
        ...
        else if (opt_key == "wall_infill_order")
        {
            ...
            else if (value == "inner-outer-inner-inner wall/infill")
            {
                opt_key = "wall_sequence";
                value = "inner-outer-inner-inner wall";
            }
            else
            {
                opt_key = "wall_sequence";
        ...
        else if (opt_key == "nozzle_volume_type" || opt_key == "default_nozzle_volume_type" || opt_key == "printer_extruder_variant" || opt_key == "print_extruder_variant" || opt_key == "filament_extruder_variant" || opt_key == "extruder_variant_list")
        {
                                                                            ...
                                                                            std::set<std::string> &key_set, std::string id_name, std::string variant_name, unsigned int stride, unsigned int extruder_id, NozzleVolumeType filament_nvt)
    {
        ...
        // if (extruder_nozzle_volume_count > 1)
        {
            ...
            if (extruder_id > 0 && extruder_id <= static_cast<unsigned>(extruder_count))
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: could not found extruder_type %2%, nozzle_volume_type %3%, for filament") % __LINE__ % s_keys_names_ExtruderType[extruder_type] % s_keys_names_NozzleVolumeType[nozzle_volume_type];
                    assert(false);
                }
            ...
            else
            {
                ...
                for (int e_index = 0; e_index < extruder_count; e_index++)
                {
                    ...
                    for (int nvt_index = 0; nvt_index < nvt_count; nvt_index++)
                    {
                        ...
                        {
                            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: could not found extruder_type %2%, nozzle_volume_type %3%, extruder_index %4%, nvt_index %5%, nvt_count %6%") % __LINE__ % s_keys_names_ExtruderType[extruder_type] % s_keys_names_NozzleVolumeType[nozzle_volume_type] % (e_index + 1) % nvt_index % nvt_count;
                            assert(false);
    ...
    void DynamicPrintConfig::update_values_to_printer_extruders_for_multiple_filaments(DynamicPrintConfig &printer_config, int extruder_count, int extruder_nozzle_volume_count, std::set<std::string> &key_set, std::string id_name, std::string variant_name)
    {
        ...
        // if (extruder_nozzle_volume_count > 1)
        {
            ...
            for (int f_index = 0; f_index < filament_count; f_index++)
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: could not found extruder_type %2%, nozzle_volume_type %3%, filament_index %4%, extruder index %5%") % __LINE__ % s_keys_names_ExtruderType[extruder_type] % s_keys_names_NozzleVolumeType[nozzle_volume_type] % (f_index + 1) % filament_maps[f_index];
                    assert(false);
                    // for some updates happens in a invalid state(caused by popup window)
                    // we need to avoid crash
                                                                   ...
                                                                   std::string extruder_id_name, std::string extruder_variant_name, std::set<std::string> &key_set1, std::set<std::string> &key_set2)
    {
        ...
        {
            // should not happen
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(" size of %1% = %2%, not equal to size of %3% = %4%") % extruder_variant_name % cur_variant_count % extruder_id_name % cur_extruder_ids.size();
        }
        else if ((target_extruder_ids.size() > 0) && target_variant_count != target_extruder_ids.size())
        {
            // should not happen
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(" size of %1% = %2%, not equal to size of %3% = %4%") % extruder_variant_name % target_variant_count % extruder_id_name % target_extruder_ids.size();
        }
        else
        {
            for (int i = 0; i < target_variant_count; i++)
            {
                ...
                {
                    if ((target_extruder_variants[i] == cur_extruder_variants[j]) && (target_extruder_ids.empty() || (target_extruder_ids[i] == cur_extruder_ids[j])))
                    {
                        variant_index[i] = j;
                        break;
                    }
        ...
        for (auto &opt : keys)
        {
            ...
            {
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(" change key %1% from old_value %2% to inherit's value %3%") % opt % (opt_src->serialize()) % (opt_target->serialize());
                ...
                {
                    if (opt_target->is_scalar() || ((key_set1.find(opt) == key_set1.end()) && (key_set2.empty() || (key_set2.find(opt) == key_set2.end()))))
                    {
    ...
    void DynamicPrintConfig::update_diff_values_to_child_config(DynamicPrintConfig &new_config, std::string extruder_id_name, std::string extruder_variant_name, std::set<std::string> &key_set1, std::set<std::string> &key_set2)
    {
        ...
        else
        {
            for (int i = 0; i < cur_variant_count; i++)
            {
                ...
                {
                    if ((cur_extruder_variants[i] == target_extruder_variants[j]) && (cur_extruder_ids.empty() || (cur_extruder_ids[i] == target_extruder_ids[j])))
                    {
                        variant_index[i] = j;
                        break;
                    }
        ...
        for (auto &opt : keys)
        {
            ...
            if (opt_src && opt_target && (*opt_src != *opt_target))
            {
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(" change key %1% from base_value %2% to child's value %3%") % opt % (opt_src->serialize()) % (opt_target->serialize());
                if (opt_target->is_scalar() || ((key_set1.find(opt) == key_set1.end()) && (key_set2.empty() || (key_set2.find(opt) == key_set2.end()))))
                {
                    // nothing to do, keep the original one
                    opt_src->set(opt_target);
...
#define PRINT_CONFIG_CACHE_INITIALIZE(CLASSES_SEQ)                                                              \
    ...
    Polygon get_bed_shape_with_excluded_area(const PrintConfig &cfg, bool use_share)
    {
        ...
        for (int i = 0; i < excluse_area_points.size(); i++)
        {
            ...
            if (i % 4 == 3)
            { // exclude areas are always rectangle
    ...
    bool has_skirt(const DynamicPrintConfig &cfg)
    {
        ...
        return (opt_skirt_height && opt_skirt_height->getInt() > 0 && opt_skirt_loops && opt_skirt_loops->getInt() > 0) || (opt_draft_shield && opt_draft_shield->getInt() != dsDisabled);
    }
