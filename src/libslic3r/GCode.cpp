...
#define NOZZLE_CONFIG(OPT) m_config.OPT.get_at(m_config.filament_map_2.values[m_writer.filament()->id()])
    ...
    std::vector<GCode::LayerToPrint> GCode::collect_layers_to_print(const PrintObject &object)
    {
        ...
        if (!warning_ranges.empty())
        {
            ...
            const_cast<Print *>(object.print())->active_step_add_warning(PrintStateBase::WarningLevel::CRITICAL, warning, PrintStateBase::SlicingEmptyGcodeLayers);
        }
    ...
    std::vector<std::pair<coordf_t, std::vector<GCode::LayerToPrint>>> GCode::collect_layers_to_print(const Print &print)
    {
        ...
        std::sort(ordering.begin(), ordering.end(), [](const OrderingItem &oi1, const OrderingItem &oi2)
                  { return oi1.print_z < oi2.print_z; });
    ...
    namespace DoExport
    {
        ...
        static void update_print_estimated_stats(const GCodeProcessor &processor, const std::vector<Extruder> &extruders, PrintStatistics &print_statistics)
        {
            ...
            for (auto volume : result.print_statistics.total_volumes_per_extruder)
            {
                ...
                auto extruder = std::find_if(extruders.begin(), extruders.end(), [extruder_id](const Extruder &extr)
                                             { return extr.id() == extruder_id; });
        ...
        static std::vector<std::pair<std::string, std::string>> validate_custom_gcode(const Print &print)
        {
            ...
            if (ret.size() < MAX_TAGS_COUNT)
                check(_(L("Time lapse G-code")), config.wrapping_detection_gcode.value);
            if (ret.size() < MAX_TAGS_COUNT)
    ...
    void GCode::do_export(Print *print, const char *path, GCodeProcessorResult *result, ThumbnailsGeneratorCallback thumbnail_cb)
    {
        ...
        for (const auto &filament : m_writer.extruders())
            activate_long_retraction_when_cut |= (m_config.long_retractions_when_cut.get_at(filament.id()) && m_config.retraction_distances_when_cut.get_at(filament.id()) > 0);
...
static inline std::vector<const PrintInstance*> sort_object_instances_by_max_z(const Print &print)
{
    ...
    std::vector<const PrintInstance *> sort_object_instances_by_model_order(const Print &print, bool init_order)
    {
        ...
        std::sort(model_instance_to_print_instance.begin(), model_instance_to_print_instance.end(), [](auto &l, auto &r)
                  { return l.first->arrange_order < r.first->arrange_order; });
        ...
        for (const ModelObject *model_object : print.model().objects)
            ...
            {
                auto it = std::lower_bound(model_instance_to_print_instance.begin(), model_instance_to_print_instance.end(), std::make_pair(model_instance, nullptr), [](auto &l, auto &r)
                                           { return l.first->arrange_order < r.first->arrange_order; });
                if (it != model_instance_to_print_instance.end() && it->first == model_instance)
                    instances.emplace_back(it->second);
            }
        std::sort(instances.begin(), instances.end(), [](auto &l, auto &r)
                  { return l->model_instance->arrange_order < r->model_instance->arrange_order; });
        return instances;
    }
    ...
    void GCode::_do_export(Print &print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
    {
        ...
        { // Don't support skipping in cali mode
                ...
                if (filament_count_tmp * filament_count_tmp * heads_count_tmp == temp_flush_volumes_matrix.size())
                {
                    for (size_t idx = 0; idx < heads_count_tmp; ++idx)
                    {
                        ...
                        std::transform(temp_flush_volumes_matrix.begin() + temp_begin_t,
                                       ...
                                       [temp_cfg_flush_multiplier_idx](double inputx)
                                       { return inputx * temp_cfg_flush_multiplier_idx; });
                    }
        ...
        {
            DoExport::export_thumbnails_to_file(thumbnail_cb, print.get_plate_index(), print.full_print_config().option<ConfigOptionPoints>("thumbnail_size")->values, [&file](const char *sz)
                                                { file.write(sz); }, [&print]()
                                                { print.throw_if_canceled(); });
        }
    ...
    print.throw_if_canceled();
        ...
        int max_chamber_temp = 0;
        {
            ...
            bool has_tpu_in_first_layer = std::any_of(first_layer_filaments.begin(), first_layer_filaments.end(), [&](unsigned int idx)
                                                      { return m_config.filament_type.values[idx] == "TPU"; });
        ...
        {                                                                           // hold chamber temp for flat print: Flag
            ...
            for (ObjectID print_object_ID_t : print.print_object_ids())
            {
                ...
                // brim area
                if (print.m_brimMap.find(print_object_ID_t) != print.m_brimMap.end() && !print.m_brimMap.at(print_object_ID_t).entities.empty())
                { // contain brim
        ...
        const bool prime_extruder)
    {
        ...
        // Extrude skirt at the print_z of the raft layers and normal object layers
        // not at the print_z of the interlaced support material layers.
        skirt_loops_per_extruder = first_layer ? Skirt::make_skirt_loops_per_extruder_1st_layer(print, layer_tools, m_skirt_done) : Skirt::make_skirt_loops_per_extruder_other_layers(print, layer_tools, m_skirt_done);
            ...
            for (int print_wipe_extrusions = is_anything_overridden; print_wipe_extrusions >= 0; --print_wipe_extrusions)
            {
                ...
                for (InstanceToPrint &instance_to_print : instances_to_print)
                {
                    ...
                    for (ObjectByExtruder::Island &island : instance_to_print.object_by_extruder.islands)
                    {
                        ...
                        {
                            if (!has_wipe_tower && need_insert_timelapse_gcode_for_traditional && printer_structure == PrinterStructure::psI3 && !has_insert_timelapse_gcode && has_infill(by_region_specific))
                            {
                                gcode += this->retract(false, false, auto_lift_type, true);
    ...
    file.write(gcode);
        ...
        if (need_insert_timelapse_gcode_for_traditional && !has_insert_timelapse_gcode)
        {
            ...
            if (m_support_traditional_timelapse && printer_structure == PrinterStructure::psI3 && m_config.timelapse_type.value == TimelapseType::tlTraditional)
                m_support_traditional_timelapse = false;

            // The traditional model will have flaws for multi_extruder when switching extruder
            if (m_config.nozzle_diameter.values.size() == 2 && m_support_traditional_timelapse && m_config.timelapse_type.value == TimelapseType::tlTraditional && (writer().filament() && get_extruder_id(writer().filament()->id()) != most_used_extruder) && !m_placeholder_parser.config().opt_bool("has_wipe_tower_this_layer"))
            {
                m_support_traditional_timelapse = false;
            }
            if (FILAMENT_CONFIG(retract_when_changing_layer))
    ...
    void GCode::append_full_config(const DynamicPrintConfig &cfg, std::string &str)
    {
        ...
        static const std::set<std::string_view> banned_keys({"compatible_printers"sv,
    ...
    std::string GCode::extrude_loop(ExtrusionLoop loop, std::string description, double speed)
    {
        ...
        const auto speed_for_path = [&speed, &small_peri_speed](const ExtrusionPath &path)
        {
            ...
            return !is_small_peri ? speed : (speed == -1) ? small_peri_speed
                                                          : std::min(small_peri_speed, speed);
        };
        ...
        if (enable_seam_slope)
        {
            ...
            float slope_gap = override_filament_scarf_seam_setting ? m_config.seam_slope_gap.get_abs_value(scale_(EXTRUDER_CONFIG(nozzle_diameter))) : FILAMENT_CONFIG(filament_scarf_gap).get_abs_value(scale_(EXTRUDER_CONFIG(nozzle_diameter)));
    ...
    ExtrusionPaths GCode::set_speed_transition(std::vector<ExtrusionPaths> &paths)
    {
        ...
        for (int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            {
                return std::accumulate(path.begin(), path.end(), 0.0, [](double sum, const ExtrusionPath &p)
                                       { return sum + p.length(); });
            };
    ...
    bool GCode::slowDownByHeight(double &maxSpeed, double &maxAcc, const ExtrusionPath &path)
    {
        ...
        if (do_slowdown_by_height)
        {
            ...
            if (height1 >= height2 || currentHeight > height2 || currentHeight < height1)
                do_slowdown_by_height = false;
            else
            {
    ...
    double GCode::calc_max_volumetric_speed(const double layer_height, const double line_width, const std::string co_str)
    {
        ...
        if (cs.size() != 6 || std::all_of(cs.begin(), cs.end(), [](double v)
                                          { return v == 0; }))
    ...
    std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed, bool use_seperate_speed, bool is_first_slope)
    {
        ...
        if (m_enable_cooling_markers)
        {
            if (FILAMENT_CONFIG(enable_overhang_bridge_fan))
            {
                ...
                int overhang_threshold = FILAMENT_CONFIG(overhang_fan_threshold) == Overhang_threshold_none ? Overhang_threshold_none : FILAMENT_CONFIG(overhang_fan_threshold) - 1;
        ...
        auto is_through_overhang = [this](const Polyline &travel)
        {
            ...
            for (size_t idx : idx_of_object_sorted)
            {
                for (const Point &instance_shift : objects_instances_shift[idx])
                {
                    ...
                    {
                        std::sort(layers_of_objects[idx].begin(), layers_of_objects[idx].end(), [](auto left, auto right)
                                  { return left->loverhangs_bbox.area() > right->loverhangs_bbox.area(); });
    ...
    std::string GCode::set_extruder(unsigned int new_filament_id, double print_z, bool by_object)
    {
        ...
        {
            int temp = (m_layer_index <= 0 ? m_config.nozzle_temperature_initial_layer.get_at(new_filament_id) : m_config.nozzle_temperature.get_at(new_filament_id));
