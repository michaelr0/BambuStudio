...
namespace Slic3r
{
    ...
    public:
        ...
        PerimeterGeneratorLoop(const Polygon &polygon, unsigned short depth, bool is_contour, bool is_small_width_perimeter = false, bool need_circle_compensation_ = false) : polygon(polygon), is_contour(is_contour), is_smaller_width_perimeter(is_small_width_perimeter), depth(depth), need_circle_compensation(need_circle_compensation_) {}
...
static void fuzzy_extrusion_line(Arachne::ExtrusionLine& ext_lines, double fuzzy_skin_thickness, double fuzzy_skin_point_dist)
{
    ...
    static ExtrusionEntityCollection traverse_loops(const PerimeterGenerator &perimeter_generator, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls)
    {
        ...
        for (const PerimeterGeneratorLoop &loop : loops)
        {
            ...
            if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers)
            {
                ...
                bool detect_overhang_speed = perimeter_generator.config->enable_overhang_speed.get_at(get_extruder_index(*(perimeter_generator.print_config), perimeter_generator.config->wall_filament - 1)) && perimeter_generator.config->fuzzy_skin == FuzzySkinType::None;
        ...
        for (PerimeterGeneratorArachneExtrusion &pg_extrusion : pg_extrusions)
        {
            ...
            *extrusion = apply_fuzzy_skin(*extrusion, *(perimeter_generator.config), *(perimeter_generator.perimeter_regions), perimeter_generator.layer_id,
            ...
            if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers)
            {
                ...
                if (perimeter_generator.config->enable_overhang_speed.get_at(get_extruder_index(*(perimeter_generator.print_config), perimeter_generator.config->wall_filament - 1)) && perimeter_generator.config->fuzzy_skin == FuzzySkinType::None)
                {
    ...
    void PerimeterGenerator::process_arachne()
    {
        ...
        for (const Surface &surface : this->slices->surfaces)
        {
            ...
            int new_size = std::accumulate(last.begin(), last.end(), 0, [](int prev, const ExPolygon &expoly)
                                           { return prev + expoly.num_contours(); });
            ...
            bool is_outer_wall_first =
                this->config->wall_sequence == WallSequence::OuterInner || this->config->wall_sequence == WallSequence::InnerOuterInner || this->config->wall_sequence == WallSequence::InnerOuterInnerInner;
            if (layer_id == 0)
            {
            ...
            // BBS. adjust wall generate seq
            if (this->config->wall_sequence == WallSequence::InnerOuterInner || this->config->wall_sequence == WallSequence::InnerOuterInnerInner)
            {
                if (this->config->wall_sequence == WallSequence::InnerOuterInnerInner && ordered_extrusions.size() > 3)
                {                                                               // 4 walls minimum needed to do inner outer inner inner ordering
                    int position = 0;                                           // index to run the re-ordering for multiple external perimeters in a single island.
                    int arr_i = 0;                                              // index to run through the walls
                    int outer, first_internal, second_internal, third_internal; // allocate index values
                    // run the re-ordering for all wall loops in the same island
                    while (position < ordered_extrusions.size())
                    {
                        outer = first_internal = second_internal = third_internal = -1; // initialise all index values to -1
                        ...
                        for (arr_i = position; arr_i < ordered_extrusions.size(); ++arr_i)
                        {
                            ...
                            case 2: // second internal wall
                                ...
                                break;
                            case 3: // second internal wall
                                if (ordered_extrusions[arr_i].extrusion->inset_idx == 3 && third_internal == -1 &&
                                    arr_i > second_internal && outer != -1)
                                    third_internal = arr_i;
                                break;
                            }
                        }
                        if (outer > -1 && first_internal > -1 && second_internal > -1 && third_internal > -1)
                            break; // found all three perimeters to re-order
                    }
                    if (outer > -1 && first_internal > -1 && second_internal > -1 && third_internal > -1)
                    { // found perimeters to re-order?
                        const auto temp = ordered_extrusions[third_internal];
                        ordered_extrusions[third_internal] = ordered_extrusions[second_internal];
