                ...
                {                                               // 3 walls minimum needed to do inner outer inner ordering
                    ...
                    while (position < ordered_extrusions.size())
                    {
                        ...
                        { // found perimeters to re-order?
                            ...
                            if (ordered_extrusions.size() > 3 && this->config->is_outer_second)
                            {
                                if (outer != 1)
                                {
                                    auto outer_wall = ordered_extrusions[outer];
                                    ordered_extrusions.erase(ordered_extrusions.begin() + outer);
                                    ordered_extrusions.insert(ordered_extrusions.begin() + 1, outer_wall);
                                }
                            }
                            else
                            {
