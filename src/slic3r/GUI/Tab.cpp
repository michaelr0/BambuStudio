    ...
    m_hsizer->Add(m_left_sizer, 0, wxEXPAND | wxLEFT | wxTOP | wxBOTTOM, 3);
        ...
        m_tabctrl->SetFocus();
            ...
            switch_excluder(evt.GetInt()); });
                ...
                m_wiki_bmp->Bind(wxEVT_ENTER_WINDOW, [set_hover](wxMouseEvent &)
                                 { set_hover(true); });
                m_wiki_label->Bind(wxEVT_ENTER_WINDOW, [set_hover](wxMouseEvent &)
                                   { set_hover(true); });
                m_wiki_bmp->Bind(wxEVT_LEAVE_WINDOW, [set_hover](wxMouseEvent &)
                                 { set_hover(false); });
                m_wiki_label->Bind(wxEVT_LEAVE_WINDOW, [set_hover](wxMouseEvent &)
                                   { set_hover(false); });
            ...
            m_btn_save_preset->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e)
                                                   { save_preset(); }));
            m_btn_delete_preset->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e)
                                                     { delete_preset(); }));
...
//	page->SetDoubleBuffered(true);
        ...
        void Tab::update_label_colours()
        {
            ...
            while (cur_item >= 0)
            {
                ...
                for (auto page : m_pages)
                {
                    ...
                    const wxColor *clr = !page->m_is_nonsys_values ? &m_sys_label_clr : page->m_is_modified_values ? &m_modified_label_clr
                                                                                                                   : (m_type < Preset::TYPE_COUNT ? &m_default_text_clr : &m_modified_label_clr);

                    m_tabctrl->SetItemTextColour(cur_item, clr == &m_modified_label_clr ? *clr : StateColor(std::make_pair(0x6B6B6C, (int)StateColor::NotChecked), std::make_pair(*clr, (int)StateColor::Normal)));
                    break;
                }
        ...
        void Tab::update_changed_ui()
        {
            ...
            const bool deep_compare = (m_type == Preset::TYPE_PRINTER || m_type == Preset::TYPE_PRINT || m_type == Preset::TYPE_FILAMENT || m_type == Preset::TYPE_SLA_MATERIAL || m_type == Preset::TYPE_MODEL);
        ...
        void Tab::update_changed_tree_ui()
        {
            ...
            while (cur_item >= 0)
            {
                ...
                for (auto page : m_pages)
                {
                    ...
                    const wxColor *clr = sys_page ? (m_is_default_preset ? &m_default_text_clr : &m_sys_label_clr) : (modified_page || m_type >= Preset::TYPE_COUNT) ? &m_modified_label_clr
                                                                                                                                                                     : &m_default_text_clr;
    ...
    return
        ...
        void Tab::on_value_change(const std::string &opt_key, const boost::any &value)
        {
            ...
            if (og_freq_chng_params)
            {
                ...
                if (is_fff ? (opt_key == "enable_support" || opt_key == "support_type" || opt_key == "support_on_build_plate_only") : (opt_key == "supports_enable" || opt_key == "support_buildplate_only"))
            ...
            if (opt_key == "filament_long_retractions_when_cut")
            {
                ...
                if (activate == 1)
                {
                    MessageDialog dialog(wxGetApp().plater(),
                                         _L("Experimental feature: Retracting and cutting off the filament at a greater distance during filament changes to minimize flush."
                                            "Although it can notably reduce flush, it may also elevate the risk of nozzle clogs or other printing complications.Please use with the latest printer firmware."),
                                         "", wxICON_WARNING | wxOK);
                    dialog.ShowModal();
                }
                if (wxGetApp().app_config->get("auto_calculate_flush") == "all")
    ...
    if (opt_key == "extruders_count")
        ...
        void TabPrint::build()
        {
            ...
            optgroup->append_single_option_line("is_outer_second", "parameter/quality-advance-settings");
            ...
            { validate_custom_note_cb(this, optgroup, opt_key, value); };
    ...
    //    build_preset_description_line(optgroup.get());
        ...
        static std::vector<std::string> intersect(std::vector<std::string> const &l, std::vector<std::string> const &r)
        {
            ...
            std::copy_if(r.begin(), r.end(), std::back_inserter(t), [&l](auto &e)
        ...
        {
            std::vector<std::string> t;
            std::copy_if(l.begin(), l.end(), std::back_inserter(t), [&r](auto &e)
                         { return std::find(r.begin(), r.end(), e) == r.end(); });
            return t;
        }

        TabPrintModel::TabPrintModel(ParamsPanel *parent, std::vector<std::string> const &keys)
            : TabPrint(parent, Preset::TYPE_MODEL), m_keys(intersect(Preset::print_options(), keys)), m_prints(Preset::TYPE_MODEL, Preset::print_options(), static_cast<const PrintRegionConfig &>(FullPrintConfig::defaults()))
        {
        ...
        void TabPrintModel::build()
        {
            ...
            for (auto p : m_pages)
            {
                ...
                {
                    g->remove_option_if([this](auto &key)
                                        { return !has_key(key); });
                    g->have_sys_config = [this]
                    { m_back_to_sys = true; return true; };
                }
        ...
        void TabFilament::update_filament_overrides_page()
        {
            ...
            const auto og_it = std::find_if(page->m_optgroups.begin(), page->m_optgroups.end(), [](const ConfigOptionsGroupShp og)
                                            { return og->title == "Retraction"; });
    ...
    //    build_preset_description_line(optgroup.get());
        ...
        void TabPrinter::build_fff()
        {
            ...
            m_sys_extruders_count = parent_preset == nullptr ? 0 : static_cast<const ConfigOptionFloatsNullable *>(parent_preset->config.option("nozzle_diameter"))->values.size();
    ...
    if (from_initial_build ||
        ...
        void TabPrinter::on_preset_loaded()
        {
            ...
            if (base_name != m_base_preset_name)
            {
                ...
                if (base_model != m_base_preset_model)
                {
                    ...
                    bool has_multiple_nozzle = std::any_of(extruder_max_nozzle_count->values.begin(), extruder_max_nozzle_count->values.end(), [](int i)
                                                           { return i > 1; });
        ...
        void TabPrinter::toggle_options()
        {
            ...
            {
                assert(m_config->option<ConfigOptionEnum<GCodeFlavor>>("gcode_flavor")->value == gcfMarlinLegacy || m_config->option<ConfigOptionEnum<GCodeFlavor>>("gcode_flavor")->value == gcfMarlinFirmware || m_config->option<ConfigOptionEnum<GCodeFlavor>>("gcode_flavor")->value == gcfKlipper);
    ...
    wxTheApp->CallAfter([this]
#endif
            {
                ...
                if (m_type == Slic3r::Preset::TYPE_PRINTER)
                {
                    ...
                    if (printer_technology != static_cast<TabPrinter *>(this)->m_printer_technology)
                    {
                        ...
                        for (auto tab : wxGetApp().tabs_list)
                        {
                            ...
                            if (tab->supports_printer_technology(printer_technology))
                            {
                                ...
                                if (!wxGetApp().tabs_as_menu())
                                {
                                    std::string bmp_name = tab->type() == Slic3r::Preset::TYPE_FILAMENT ? "spool" : tab->type() == Slic3r::Preset::TYPE_SLA_MATERIAL ? ""
                                                                                                                                                                     : "cog";
                    ...
                    if (printer_technology == ptFFF)
                    {
                        ...
                        static_cast<TabPrinter *>(this)->m_sys_extruders_count = parent_preset == nullptr ? 0 : static_cast<const ConfigOptionFloatsNullable *>(parent_preset->config.option("nozzle_diameter"))->values.size();
                    }
                }
                else
                {
        ...
        void Tab::rebuild_page_tree()
        {
            ...
            for (auto p : m_pages)
            {
                ...
                m_tabctrl->SetItemTextColour(itemId, p->get_item_colour() == m_modified_label_clr ? p->get_item_colour() : StateColor(std::make_pair(0x6B6B6C, (int)StateColor::NotChecked), std::make_pair(p->get_item_colour(), (int)StateColor::Normal)));
        ...
        void Tab::update_btns_enabling()
        {
            ...
            // and any user preset
            const Preset &preset = m_presets->get_edited_preset();
            m_btn_delete_preset->Show((m_type == Preset::TYPE_PRINTER && m_preset_bundle->physical_printers.has_selection()) || (!preset.is_default && !preset.is_system));
            ...
            std::string preset_name, bool delete_current /*=false*/, const std::string &last_selected_ph_printer_name /* =""*/, bool force_select, bool force_no_transfer)
        {
            ...
            if (preset_name.empty())
            {
                if (delete_current)
                {
                    ...
                    if (preset_name.empty())
                    {
                        ...
                        BOOST_LOG_TRIVIAL(info) << boost::format("cause by delete current ,choose the next visible, idx %1%, name %2%") % idx_new % preset_name;
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("not cause by delete current ,choose the first visible, idx %1%, name %2%") % idx_new % preset_name;
                }
            }
            ...
            else if (print_tab)
            {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("select process, new_preset_compatible %1%, old_preset_dirty %2%, cancelled %3%") % new_preset_compatible % old_preset_dirty % canceled;
            }
            else if (printer_tab)
            {
            ...
            else
            {
                ...
                {
                    return (delete_current || technology_changed) ? PresetSelectCompatibleType::Always : on_page                 ? PresetSelectCompatibleType::Never
...
// #endif
        ...
        void Tab::delete_preset()
        {
            ...
            { // root preset
                is_base_preset = true;
                if (current_preset.type == Preset::Type::TYPE_PRINTER && !current_preset.is_system)
                { // Customize third-party printers
                    Preset &current_preset = m_presets->get_selected_preset();
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("delete preset %1%, setting_id %2%, user_id %3%, base_id %4%, sync_info %5%, type %6%") % current_preset.name % current_preset.setting_id % current_preset.user_id % current_preset.base_id % current_preset.sync_info % Preset::get_type_string(m_type);
        ...
        void Tab::show_wiki()
        {
            if (m_wiki_bmp && m_wiki_label)
            {
                ...
                bool has_direct = std::any_of(variants.begin(), variants.end(), [](const std::string &v)
                                              { return v.find("Direct Drive") != std::string::npos; });
        ...
        void Tab::sync_excluder()
        {
            ...
            wxString header = active_index == 0 ? _L("Do you want to modify the following parameters of the right nozzle to that of the left nozzle?") : _L("Do you want to modify the following parameters of the left nozzle to that of the right nozzle?");
        ...
        // BBS: GUI refactor
        Page::Page(wxWindow *parent, const wxString &title, int iconID, wxPanel *tab_owner) : m_tab_owner(tab_owner),
                                                                                              m_parent(parent),
                                                                                              m_title(title),
                                                                                              m_iconID(iconID)
        {
